#!/bin/bash
daemon_cmd='/usr/local/bin/imcenter /usr/local/bin/imcenter.xml'
function stop_imcenter {
    if ! pgrep -f "$daemon_cmd"; then
        printf "Error: imcenter is not running!!\n"
        return 1
    fi
    ...
    if pgrep -f '/usr/bin/python /usr/bin/supervisord'; then
        ...
        sleep 5
        if pgrep -f "$daemon_cmd"; then
            printf "Error: failed to kill imcenter\n"
            return 2
        else
            return 0
        fi
...
function start_imcenter {
    if pgrep -f "$daemon_cmd"; then
        printf "Error: imcenter is running!!try restart it!\n"
        return 1
    fi
    ...
    if pgrep -f '/usr/bin/python /usr/bin/supervisord'; then
        ...
        if pgrep -f "$daemon_cmd"; then
            ...
            printf "Error: imcenter failed to start\n"
            return 2
        fi
    else
...
if [ ! -e $lockfile ]; then
    ...
    touch $lockfile
    if ! stop_imcenter ; then
        printf "Error: stop process failed!! exit!"
        exit 1
    fi
    if ! start_imcenter ; then
